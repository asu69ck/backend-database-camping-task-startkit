...
(SELECT "COURSE".id FROM "COURSE" JOIN "USER" ON "COACH".user_id="USER".id WHERE "USER".name='李燕容'),
'2024-11-24 17:10:25',
'即將授課'
);
...
WHERE user_id=(SELECT id FROM "USER" WHERE name='王小明')
AND status='已使用'
GROUP BY user_id) AS USED_COURSES ON "0CREDIT_PURCHASE".user_id="USED_COURSES".user_id;
