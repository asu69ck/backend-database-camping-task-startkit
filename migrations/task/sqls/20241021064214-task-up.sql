...
('透明人','opactiy0@hexschooltest.io','USER');
...
DELETE FROM USER
WHERE 
email='opactiy0@hexschooltest.io';
...
-- 2-2. 新增：在 `CREDIT_PURCHASE` 資料表，新增三筆資料：（請使用 name 欄位做子查詢）
    ...
    -- 2. `王小明` 購買 `21 堂組合包方案`
    -- 3. `好野人` 購買 `14 堂組合包方案`
INSERT INTO CREDIT_PURCHASE(user_id,credit_pakage_id)
VALUES
...
VALUES
((SELECT COACH.id FROM COACH JOIN USER ON COACH.user_id=USER.id WHERE USER.email='lee2000@hexschooltest.io'),(SELECT id FROM SKILL WHERE name='重訓')),
((SELECT COACH.id FROM COACH JOIN USER ON COACH.user_id=USER.id WHERE USER.email='muscle@hexschooltest.io'),(SELECT id FROM SKILL WHERE name='重訓')),
((SELECT COACH.id FROM COACH JOIN USER ON COACH.user_id=USER.id WHERE USER.email='starplatinum@hexschooltest.io'),(SELECT id FROM SKILL WHERE name='重訓')),
((SELECT COACH.id FROM COACH JOIN USER ON COACH.user_id=USER.id WHERE USER.email='muscle@hexschooltest.io'),(SELECT id FROM SKILL WHERE name='瑜珈')),
((SELECT COACH.id FROM COACH JOIN USER ON COACH.user_id=USER.id WHERE USER.email='starplatinum@hexschooltest.io'),(SELECT id FROM SKILL WHERE name='有氧運動')),
((SELECT COACH.id FROM COACH JOIN USER ON COACH.user_id=USER.id WHERE USER.email='starplatinum@hexschooltest.io'),(SELECT id FROM SKILL WHERE name='復健訓練'));
...
VALUES
((SELECT user_id FROM USER WHERE email='lee2000@hexschooltest.io'),(SELECT id FROM SKILL WHERE name='重訓'),'重訓基礎課','2024-11-25 14:00:00','2024-11-25 16:00:00',10,'https://test-meeting.test.io');
...
VALUES
((SELECT id FROM USER WHERE name='王小明'),(SELECT COURSE.id FROM COURSE JOIN USER ON COACH.user_id=USER.id WHERE USER.name='李燕容'),'2024-11-24 16:00:00','即將授課'),
((SELECT id FROM USER WHERE name='好野人'),(SELECT COURSE.id FROM COURSE JOIN USER ON COACH.user_id=USER.id WHERE USER.name='李燕容'),'2024-11-24 16:00:00','即將授課');
...
(SELECT COIRSE.id FROM COURSE JOIN USER ON COACH.user_id=USER.id WHERE USER.name='李燕容'),
'2024-11-24 17:10:25',
'即將授課'
);
...
SELECT *
FROM COURSE_BOOKING
WHERE user_id IN (SELECT id FROM USER WHERE name='王小明');
...
booking_at='2024-11-24 17:10:25';
-- 5-6. 查詢：計算用戶王小明的購買堂數，顯示須包含以下欄位： user_id , total。 (需使用到 SUM 函式與 Group By)
SELECT
user_id,
SUM(purchase_credits) AS total
FROM CHOURSE_PURCHASE
WHERE user_id=(SELECT id FROM USER WHERE name='王小明')
GROUP BY user_id;
...
(SUM(CREDIT_PURCHASE.total_credit)-COALESCE(SUM(COURSE_BOOKING.used_credit),0)) AS remaining_credit
FROM
(SELECT user_id,
...
WHERE user_id=(SELECT id FROM USER WHERE name='王小明')
AND status='已使用'
GROUP BY user_id) AS USED_COURSES ON CREDIT_PURCHASE.user_id=USED_COURSES.user_id;
...
INNER JOIN COACH_LINK_SKILL ON coach_id=COACH.id
...
COUNT(credit_pakage_id) AS 銷售數量
FROM CREDIT_PURCHASE
INNER JOIN CREDIT_PACKAGE ON id=CREDIT_PURCHASE.credit_pakage_id
GROUP BY CREDIT_PACKAGE.name;
-- 6-4. 查詢：計算 11 月份總營收（使用 purchase_at 欄位統計）
-- 顯示須包含以下欄位： 總營收
SELECT
SUM(prcie_paid) AS 總營收
FROM CREDIT_PURCHASE
